...
void doJSSF3DMenu()
{
    ...
    if (updateSolver && !is_calc_frame)
    {
        if (grid_smoke3d == NULL)
            grid_smoke3d = new jfs::gridSmoke3D(grid_size, grid_length, fluid_btype, iter_per_frame * dt, smoke_diss);
        else
            grid_smoke3d->initialize(grid_size, grid_length, fluid_btype, iter_per_frame * dt, smoke_diss);

        if (!is_updating)
        {
            auto initLambda = [](){jssf_solver_3d->initialize(grid_size, grid_length, fluid_btype, dt, visc); };
